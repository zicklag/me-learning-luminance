use luminance::{
    context::GraphicsContext, pipeline::PipelineState, render_state::RenderState, shader::Program,
    tess::Tess,
};
...
fn init_renderer(mut surface: Surface) -> Renderer {
    ...
    let program = surface
        ...
        .from_strings(
            include_str!("hello_triangle/shader.vert"),
            None,
            None,
            include_str!("hello_triangle/shader.frag"),
        )
        .unwrap()
        .ignore_warnings();
